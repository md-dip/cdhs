...
-- ---------------------------------------------------------------------------
-- keepalive: a single row that the daily "Keep Supabase awake" GitHub Action
-- (see .github/workflows/keep-supabase-awake.yml) writes to. A plain anon SELECT
-- turned out not to count as "activity" for Supabase's 7-day free-tier pause
-- check, so this gives the workflow a real write to perform instead — via a
-- SECURITY DEFINER function, so the workflow only ever needs the public
-- anon/publishable key (never the secret service-role key).
-- ---------------------------------------------------------------------------
...
-- Intentionally no policies: nobody can SELECT/UPDATE this table directly, not
-- even anon. The only way to touch it is through keepalive_ping() below.
